...
from RLPlayground.tune.tune_cartpole import DeepTDTrainable

if __name__ == '__main__':
    # load initial configs for params
    cfg = evaluate_file(f'{CONFIG_DIR}/pg_config.jsonnet')
    cfg = json.loads(cfg)

    tune_cfg = evaluate_file(f'{CONFIG_DIR}/tune_cartpole_config.jsonnet')
    ...
    for env_name in env_names:
        for agent in agents:
            agent_cfg[env_name][agent]['seed'] = 1337

            # select tuning params with grid search
            agent_cfg[env_name][agent]['lr'] = tune.choice(experiment_cfg['lrs'])
            if 'REINFORCE' not in agent:
                agent_cfg[env_name][agent]['entropy_weight'] = tune.choice(
                    experiment_cfg['entropy_weights'])

            # pass config along with optimizing params
            config = {'env_name': env_name, 'agent': agent,
                      'consecutive_steps_to_solve': agent_cfg[env_name][
